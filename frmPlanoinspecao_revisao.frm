...
Begin VB.Form frmPlanoinspecao_revisao 
   BackColor       =   &H00E0E0E0&
   Caption         =   "Qualidade - Plano de inspeção - Histórico de revisões"
   ClientHeight    =   10035
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   15360
...
Formulario = "Qualidade/Plano de inspeção/Histórico de revisões"
...
1:
            ...
            Documento = "Plano: " & .txtPI & " - Rev.: " & .txtRev & " - Cód. interno: " & .txtdesenho & " - Rev.: " & .txtRev_item
            Documento1 = ""
            ProcGravaEvento
            '==================================
        End If
...
With frmPlanoinspecao
    If .txtIDFase <> "" And .txtIDFase <> "0" Then
        If .FunVerifiProcRevisado(.txtIDFase, "criar nova revisão deste plano", True) = True Then Exit Sub
    End If
End With
ProcLimpaCampos
...
With frmPlanoinspecao
    ...
    If TBHistProc.EOF = True Then
        TBHistProc.AddNew
    Else
        If FunVerifValidacaoRegistro("alterar", .txtDtValidacao, "plano", "a revisão do plano", True) = False Then Exit Sub
        With frmPlanoinspecao
            If .txtIDFase <> "" And .txtIDFase <> "0" Then
                If .FunVerifiProcRevisado(.txtIDFase, "alterar a revisão deste plano", True) = True Then Exit Sub
            End If
        End With
    End If
        ...
        For InitFor = 1 To .ListItems.Count
            ...
            Else
                ...
                If frmPlanoinspecao.txtIDFase <> "" And frmPlanoinspecao.txtIDFase <> "0" Then
                    If frmPlanoinspecao.FunVerifiProcRevisado(frmPlanoinspecao.txtIDFase, "excluir a revisão deste plano", False) = True Then GoTo Proximo
                End If
...
With ListaHistorico
    For InitFor = 1 To .ListItems.Count
        If .ListItems.Item(InitFor).Checked = True And .ListItems.Item(InitFor) = Item Then
            If FunVerificaRegistroValidado("Plano", "IDPlano = " & frmPlanoinspecao.txtPI, "plano", "revisão do plano", "excluir esta", True, True) = False Then
                .ListItems.Item(InitFor).Checked = False
                Exit Sub
            End If
            If frmPlanoinspecao.txtIDFase <> "" And frmPlanoinspecao.txtIDFase <> "0" Then
                If frmPlanoinspecao.FunVerifiProcRevisado(frmPlanoinspecao.txtIDFase, "excluir a revisão deste plano", True) = True Then .ListItems.Item(InitFor).Checked = False
            End If
        End If
    Next InitFor
End With
...
txtId = TBHistProc!ID
Caption = "Qualidade - Plano de inspeção - Histórico de revisões - (Revisão : " & IIf(IsNull(TBHistProc!Revisao), "", TBHistProc!Revisao) & ")"
