...
If TBUsuarios.EOF = False Then
    ...
    If TBAcessos.EOF = True Then
        USMsgBox ("Atenção usuário " & txtUsuario & ", você não tem autorização para " & Mensagem & "."), vbExclamation, "CAPRIND v5.0"
        TBAcessos.Close
        Exit Sub
    End If
    TBAcessos.Close
    If TBUsuarios!Bloqueado = True Then
        USMsgBox ("Não é permitido efetuar " & Mesagem1 & ", pois o usuário " & txtUsuario.Text & " está bloqueado."), vbExclamation, "CAPRIND v5.0"
        TBUsuarios.Close
        Exit Sub
    End If
Else
...
Unload Me
If Sit_REG = 1 Then
    USMsgBox ("Os dados da nota fiscal serão salvos, mais a mesma não será cancelada."), vbInformation, "CAPRIND v5.0"
