...
Begin VB.Form frmEstoque_Localarmaz_bloq 
   BackColor       =   &H00E0E0E0&
   BorderStyle     =   4  'Fixed ToolWindow
   Caption         =   "Estoque - Local de armazenamento - Status"
   ClientHeight    =   3930
   ClientLeft      =   45
   ClientTop       =   390
   ClientWidth     =   6420
   ...
   Begin DrawSuite2022.USImageList USImageList1 
      ...
      _ExtentX        =   900
      _ExtentY        =   767
      Img1            =   "frmEstoque_Localarmaz_bloq.frx":0000
      Count           =   1
   End
   ...
   End
End
Attribute VB_Name = "frmEstoque_Localarmaz_bloq"
...
If txtStatus = "" Then
    USMsgBox ("Informe se o local de armazenamento está bloqueado ou liberado antes de salvar."), vbExclamation, "CAPRIND v5.0"
    Exit Sub
End If

With frmEstoque_Localarmaz
    For InitFor = 1 To .Lista_locarmazenamento.ListItems.Count
        If .Lista_locarmazenamento.ListItems.Item(InitFor).Checked = True Then
            Set TBClientes = CreateObject("adodb.recordset")
            TBClientes.Open "Select * from Estoque_Localarmazenamento_criar where id = " & .Lista_locarmazenamento.ListItems.Item(InitFor), Conexao, adOpenKeyset, adLockOptimistic
            If TBClientes.EOF = False Then
                If txtStatus.Text = "Liberada" Then
                    TBClientes!status = "Liberado"
                    Evento = "Liberar local de armazenamento"
                Else
                    TBClientes!status = "Bloqueado"
                    Evento = "Bloquear local de armazenamento"
                End If
                ...
                ID_documento = TBClientes!ID
                Documento = "Local de armazenamento: " & TBClientes!Descricao
                Documento1 = ""
                ProcGravaEvento
                '==================================
            End If
...
With frmEstoque_Localarmaz
    ...
    If contador = 1 Then
        Set TBClientes = CreateObject("adodb.recordset")
        TBClientes.Open "Select * from Estoque_Localarmazenamento_criar where ID = " & IIf(.txtID = "", 0, .txtID), Conexao, adOpenKeyset, adLockOptimistic
        If TBClientes.EOF = False Then
