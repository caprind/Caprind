    ...
    For InitFor = 1 To .ListItems.Count
        If .ListItems.Item(InitFor).Checked = True Then
            If Permitido = False Then
                If USMsgBox("Deseja realmente excluir este(s) texto(s) padrão?", vbYesNo, "CAPRIND v5.0") = vbYes Then GoTo 1 Else Exit Sub
            End If
1:
            Permitido = True
...
End With
If Permitido = False Then
    USMsgBox ("Informe o(s) texto(s) padrão antes de excluir."), vbExclamation, "CAPRIND v5.0"
Else
    USMsgBox ("Texto(s) padrão excluído(s) com sucesso."), vbInformation, "CAPRIND v5.0"
    ProcLimpaCampos
